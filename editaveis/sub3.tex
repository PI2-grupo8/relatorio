...
como serão processados e armazenados os dados coletados das medições.
  ...
  \subsection{Algoritmo de Locomoção}
    Dado a utilização de sensores infravermelho/ultrasônico e a disposição da lavoura de morango
    é possível identificar um típico problema denominado \textit{Wall Follower}.

    Segundo \cite{huang2009}, \textit{Wall Follower},
    também conhecido como regra da mão direita ou esquerda, é válida para percursos conexos, onde todas as paredes são ligadas entre si, mantendo a mão próxima a parede do labirinto em

...
como serão processados e armazenados os dados coletados das medições.
  ...
  Posteriormente depois de uma análise do projeto pela supervisão, o meio utilizado para armazenamento dos dados, será o envio dos mesmos via wireless através do protocolo ftp.
  ...
  Normalmente o acesso é controlado através de um login e senha. No servidor é possível configurar quais pastas devem ficar disponíveis para cada usuário e especificar as permissões de cada um (ler, escrever, listar, etc.). Existem ainda a opção de criar um login anônimo, com permissões restritas, que pode ser usado para compartilhar dados de acesso público.
  
  Com esse protocolo, utilizamos para enviar os arquivos com informações que servirão para fazer parte da orientação do carro e também obter o arquivo com as informações dos dados coletados de temperatura do ar, umidade do ar e do solo.
